...
using bitcoin::TransactionOutput;
using bitcoin::TransactionInput;
using bitcoin::TransactionBody;
using nlohmann::json;
...
TEST_CASE("Test parse json") {
  ...
  SECTION("Test read simple data") {
    REQUIRE(j["op"] == "utx");
  };

  SECTION("test create transaction object") {
    auto p1 = j.get<Transaction>();
    REQUIRE(p1.op == "utx");
    REQUIRE(p1.x.lock_time == 0);
    REQUIRE(p1.x.inputs[0].sequence == 4294967295);
    REQUIRE(p1.x.out[0].spent == false);
  }
}